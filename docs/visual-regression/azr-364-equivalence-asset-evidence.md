...
- Chunk, batch, or aggregate boundary used: the existing entity WebGL atlas
  page batch remains the draw boundary. Settlement, vegetation, citizen,
  stockpile, work-status, material/effect, and world-UI facades select bounded
  accepted-use cells before instance submission.
...
- Player-perception contract preserved: settlement/ground zoom gains denser
  accepted structure, vegetation, citizen, stockpile, status, effect, and
  world-UI detail without changing aggregate settlement authority, zoom
  movement, or direct-file launch.
...
- Metric proving the bottleneck moved: the `settlement-ground` visual gate now
  ...
  vegetation, citizen, stockpile, work-status, material/effect, and world UI.
